...
if uploaded_file is not None:
    ...
    input_prompt_1 = """
        You are an experienced HR professional with technical expertise in Data Science, Full Stack Development, Web Development, or Machine Learning.
    ...
    if Submit_1:
        ...
        response = get_gemini_response(input_text, resume_text, input_prompt_1)
        st.subheader("The Response is")
        st.write(response)
        
    if Submit_2:
        resume_text = extract_text_from_pdf(uploaded_file)
        response = get_gemini_response(input_text, resume_text, input_prompt_2)
        st.subheader("The Response is")
        st.write(response)
    ...
    if Submit_3:
        resume_text = extract_text_from_pdf(uploaded_file)
        response = get_gemini_response(input_text, resume_text, input_prompt_3)
        st.subheader("The Response is")
        st.write(response)

    if Submit_4:
        resume_text = extract_text_from_pdf(uploaded_file)
        response = get_gemini_response(input_text, resume_text, input_prompt_4)
        st.subheader("The Response is")
        st.write(response)
    
    if Submit_5:
        resume_text = extract_text_from_pdf(uploaded_file)
        response = get_gemini_response(input_text, resume_text, input_prompt_5)
        st.subheader("The Response is")
        st.write(response)
        
    if Submit_6:
        resume_text = extract_text_from_pdf(uploaded_file)
        response = get_gemini_response(input_text, resume_text, input_prompt_6)
        # Get Personalized Learning Path
        learning_path_response = get_gemini_response(input_text, resume_text, input_prompt_6)
        st.subheader("Personalized Learning Path")
        st.write(learning_path_response)
